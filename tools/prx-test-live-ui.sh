...
step CRON-CREATE 0 script -q -e -c "$wrapper $v $c $core_c --backup-dir $work/cron-backups --files-cnt 1 --cron" /dev/null <<<$'4\ny'
...
step CRON-ROTATION 0 test "$(find "$work/cron-backups" -name '*-dmp.tar.gz' | wc -l)" = 1
hot="$(find "$work/cron-backups" -name '*-dmp.tar.gz' -print -quit)"
...
cleanup_ui_artifacts
mkdir "$work/real-backups"
ln -s "$work/real-backups" "$work/backup-link"
