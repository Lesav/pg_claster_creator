...
- Повышена patch-версия после исправлений журналирования установки SQL-пакетов
  и упаковки SQL-деревьев со ссылками и пробелами в именах.
- SQL-пакеты сохраняют объединённый вывод установки и `psql` в приватном
  `/tmp/<полное-имя-DEB>-YYYY-MM-DD-hhmmss.log`, не скрывая исходную
  ошибку в терминале; при отказе выводятся код и точный путь к журналу.
- Сборщик сохраняет логическое написание `--sql-file` в repeat-сценарии и
  обходит ссылки на каталоги SQL-дерева (включая Windows Junction из WSL) под
  именем ссылки. Их SQL включается в DEB обычными файлами; цикл ссылок запрещён.
  Пробелы во внутренних путях DEB кодируются как `%20` без изменения SQL.
- Версия 2.6.1 прошла разрушающий прогон на восьми WSL: все коды завершения 0,
  3808 PASS и 120 сверок ожидаемого rc. Порядок одиночного SQL/дерева,
  восстановление пакетов и очистка подтверждены. В журнале 27/61 ID покрыты
  полностью, 34 частично; запрошенный суффикс `passed` не означает выполнения
  явно перечисленных пропущенных вариантов.
