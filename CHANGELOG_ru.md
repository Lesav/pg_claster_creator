...
- Вложения релизов GitHub и GitFlic используют имя `claster-creator-X.Y.Z.sha256`. CI сохраняет `SHA256SUMS` только как совместимую копию в артефакте сборки. GitFlic отправляет файлы контрольных сумм с типом `text/plain`; для GitFlic 4.5.0 дополнительно нужен отдельный серверный патч проверки имён файлов контрольных сумм. Существующие вложения релизов и теги не изменены.
