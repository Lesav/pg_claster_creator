...
- На GitFlic 4.5.0 подтверждены создание релиза с автором и явный запуск тегового конвейера пользовательским токеном. Вложения DEB по-прежнему блокируются зашитым в сервер списком MIME-типов; полная автоматическая публикация ещё не подтверждена.
